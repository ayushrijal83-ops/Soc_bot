...
class TestMenuHandler:
    ...
    def test_handle_choice_content_inbox(self):
        ...
        assert result is True
        mock_print.assert_called_once_with("Content Inbox")

    def test_prompt_does_not_add_a_second_exit(self):
        """Regression: allow_exit=True added a dead extra 'Exit' option after the real one."""
        handler = MenuHandler()
        with patch('src.cli.menu.prompt_menu_selection', return_value=None) as prompt:
            handler.run()
        assert prompt.call_args.kwargs["allow_exit"] is False
